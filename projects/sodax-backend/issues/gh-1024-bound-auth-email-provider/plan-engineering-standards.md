...
- **S3 — Discriminate state by `kind`, don't invent parallel boolean flags.** Precedent: the existing wallet-modal state machine (`closed → chainSelect → walletSelect → connecting → success|error`) and its own rule: *"Surface raw `Error` objects; do not introduce a feature-specific error taxonomy without a broader design decision."* `SodaxAuthModal` follows the identical shape.
